...
def download(url: str, fmt: str, outdir: str, quality: str = 'best', out_format: str = None, samplerate: int = None, allow_playlist: bool = False):
    import subprocess
    import re
    # If caller asked to allow playlists but provided a video URL that contains
    # a playlist id (e.g. a URL like ...watch?v=...&list=PL...), normalize it to
    # a playlist URL so yt-dlp will download all entries instead of a single
    # video.
    try:
        if allow_playlist and url:
            parsed = urlparse(url)
            qs = parse_qs(parsed.query)
            if 'list' in qs and 'playlist' not in url.lower():
                listid = qs['list'][0]
                url = f"https://www.youtube.com/playlist?list={listid}"
    except Exception:
    ...
    if target == 'video' or (target is None and fmt == 'mp4'):
        ...
        if out_format and out_format.lower() in audio_exts:
            ...
            if quality and quality != 'best':
                ...
                if m:
                    pref = m.group(1)
            # yt-dlp's FFmpegExtractAudio expects codec names like 'vorbis'
            # (not the container name 'ogg'). Map common container names
            # to the codec identifiers yt-dlp recognizes.
            codec_aliases = {'ogg': 'vorbis'}
            preferred = codec_aliases.get(out_format.lower(), out_format.lower())
            post = {
    ...
    with YoutubeDL(ydl_opts) as ydl:
        ...
        # If a playlist was downloaded, yt-dlp handles per-entry downloading
        # and postprocessing. In that case, `info` will be a playlist dict
        # (has `_type` == 'playlist' and an `entries` list). Avoid treating
        # the playlist as a single file for subsequent conversion steps;
        # return early so each entry is left as yt-dlp produced it.
...
def main():
    ...
    parser.add_argument('--playlist', action='store_true', help='Allow downloading a playlist URL')
    ...
    try:
        download(args.url, args.format, args.out, args.quality, args.out_format, args.samplerate, allow_playlist=args.playlist)
        print('Done.')
